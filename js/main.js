...
(function () {
  ...
  // Lv 칭호 — SITE_CONFIG.rankTitles가 있으면 그걸 쓰고, 없으면 본진 기본값(길이는 MAX_LEVEL+1이어야 함).
  ...
  function initBackground() {
    ...
    function draw() {
      ...
      } else if (bgSkin === "sunset") {
        // 노을 — 그라데이션은 CSS(--page-bg)가 담당, 캔버스는 별 한두 점만 정적으로 은은하게
        // (fxLevel calm과 어울리게 절제 — 움직임 없이 반짝임만)
        stars.slice(0, 2).forEach(s => {
          s.a += s.tw * 0.4;
          const alpha = 0.18 + Math.abs(Math.sin(s.a)) * 0.22;
          ctx.beginPath();
          ctx.fillStyle = `rgba(245,239,230,${alpha})`;       // #F5EFE6 따뜻한 미색
          ctx.arc(s.x, s.y, s.r, 0, Math.PI * 2);
          ctx.fill();
        });
      }
